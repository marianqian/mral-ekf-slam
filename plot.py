...
def plotMap(ls,hist,robot,mapsize):
    plt.clf()
...
def plotEstimate(mu, cov, robot, mapsize):
    ...
    for i in range(len(mu)):
        if i == 0 or i%2 == 1:
            a.arrow(*stateToArrow(mu[i][:3]), head_width=0.5, color=(1,0,0))
        else:
            a.arrow(*stateToArrow(mu[i][:3]), head_width=0.5, color=(0,1,0))
    
    # plot current robot field of view
    fov = robot.fov
    plt.plot([mu[-1][0], mu[-1][0]+50*np.cos(mu[-1][2] + fov/2)], [mu[-1][1], mu[-1][1]+50*np.sin(mu[-1][2] + fov/2)], color="r")
    plt.plot([mu[-1][0], mu[-1][0]+50*np.cos(mu[-1][2] - fov/2)], [mu[-1][1], mu[-1][1]+50*np.sin(mu[-1][2] - fov/2)], color="r")
    
    # plot current robot state covariance
    robot_cov = Ellipse(xy=mu[-1][:2], width=cov[0,0], height=cov[1,1], angle=0)
...
def plotError(mu,x_true):
    b = plt.subplot(133)
    mu = mu[0::2, :3] # keep only x,y,theta
